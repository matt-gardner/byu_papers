...
speculative or non-speculative.  The exact evaluation time at which this
architecture becomes reasonable depends on the amount of communication overhead
in the parallel implementation and the number of particles in the swarm.  For
our implementation we found that time to be around 100 milliseconds for swarms
of 50 particles.  When the swarm size increases, the minimum evaluation time at
...
are the direct equivalent of wall-clock time.  It is true that with a fixed
number of processors, function evaluations are also equivalent to wall-clock
time.  However, reporting function evaluations in a paper on parallel PSO can
be misleading if compared to a paper on serial PSO, as the actual wall-clock
times corresponding to the function evaluations are not directly comparable.
Thus we report iterations in our results instead of function evaluations.  But
because SEPSO actually performs two iterations of PSO at each ``iteration,''
and Social Promotion and Many Iterations make the idea of ``iterations of PSO''
somewhat nebulus, we instead call each ``iteration'' a ``time step.''
...
time steps.  That being said, given the time required for the evaluation of the
objective function and the communication overhead per iteration for a specific
implementation, an actual running time can be recovered from all of our
...
One might question the use of benchmark functions for evaluating our methods,
as they take on the order of 1 millisecond to evaluate instead of the 100s that
we said were required to justify our parallel architecture.  The purpose of
...
To aid in convincing the reader that performance on benchmark functions roughly
...
successful, though it is only successful 84\% of the time.  \tabref{griewank}
shows all of our results in tabular form.
...
\begin{figure}
  ...
  \caption{Function Griewank with a swarm that uses 240 processors per time
  step, comparing Pick Best with results from \fig{basic-griewank2}.  All
...
\begin{figure}
  ...
  \caption{Function Griewank with a swarm that uses 240 processors per time
  step, comparing pruning with the best results from \fig{griewank-pickbest}.
...
\begin{figure}
  ...
  \caption{Function Griewank with a swarm that uses 240 processors per time
  step, comparing speculating several iterations ahead with selections from
  \fig{griewank-pruned}.  All algorithms use the Ring topology.}
...
any significant progress.  Our search for an explanation seems to say that the
...
minutes to reach a successful value, while Standard takes 50 minutes.
