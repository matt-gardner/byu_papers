...
outperforms the naive parallelization of simply adding particles to the
...
we can introduce new speculative techniques that often out-perform naive
...
If experimentation with benchmark functions provides sufficient experimental
evidence for algorithmic improvements in serial PSO, it is also sufficient
evidence for algorithmic improvements in parallel PSO.  While the discussion of
exact running times of function evaluations and communication overhead is
certainly appropriate, even essential, to a discussion of specific parallel
implementations, the concern of this paper is more fundamental than
implementation decisions.  No one expects to see actual running times in place
of function evaluations in a paper describing an innovation in the motion
equations of PSO.  Similarly, actual running times should not be required of
those describing algorithmic improvements in parallel PSO.  Just as in a serial
PSO paper, actual running times cloud the more important issues of
implementation-independent properties of the innovations described.
...
\begin{figure}
  \centering
  \includegraphics[width=.7\columnwidth]{griewank2}
