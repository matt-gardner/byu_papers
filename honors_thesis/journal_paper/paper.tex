...
\begin{algorithm} [h!]
  ...
  \begin{algorithmic}[1]
    ...
    \REPEAT
        \STATE $t=t+1$
        \FOR{$i \in \swarm$}
          \STATE {$\fppos_{i.t} \leftarrow \ofunc(\ppos_{i,t})$}
        \ENDFOR
        \FOR{$i \in \swarm$}
          \STATE $\nURand_{i,t} \Leftarrow RandomVector()$
          \STATE $\nURand_{i,t} \Leftarrow RandomVector()$
          \label{alg1:startifs}
          \STATE $\pbest_{i,t}\leftarrow \pbest_{i,t-1}$
          \STATE $\fpbest_{i,t}\leftarrow \fpbest_{i,t-1}$
          \IF {$\fppos_{i,t}<\fpbest_{i,t-1}$}
            \STATE $\pbest_{i,t}\leftarrow \ppos_{i,t}$
            \STATE $\fpbest_{i,t}\leftarrow \fppos_{i,t}$
          \ENDIF
          \STATE $\nbest_{i,t}\leftarrow \nbest_{i,t-1}$
          \STATE $\fnbest_{i,t}\leftarrow \fnbest_{i,t-1}$
          \IF {$\fppos_{i,t}<\fnbest_{i,t}$}
            \STATE $\pbest_{i,t}\leftarrow \ppos_{i,t}$
            \STATE $\fpbest_{i,t}\leftarrow \fppos_{i,t}$
          \ENDIF
          \IF {$\fppos_{\ln(i),t}<\fnbest_{i,t}$}
            \STATE $\pbest_{i,t}\leftarrow \ppos_{\ln(i),t}$
            \STATE $\fpbest_{i,t}\leftarrow \fppos_{\ln(i),t}$
          \ENDIF
          \IF {$\ofunc(\ppos_{i,t})<\ofunc(\nbest_{\rn(i),t})$}
            \STATE $\pbest_{i,t}\leftarrow \ppos_{\rn(i),t}$
            \STATE $\fpbest_{i,t}\leftarrow \fppos_{\rn(i),t}$
          \label{alg1:endifs}
          \ENDIF
          \STATE Update $\pvel_{i,t+1}$ as shown in equation~\eqref{eq:velupdate} 
          \STATE Update $\ppos_{i,t+1}$ as shown in equation~\eqref{eq:posupdate}
        \ENDFOR
    \UNTIL {$t >$ max iteration or convergence criteria met}
  \end{algorithmic}
\end{algorithm}

This algorithm can be transformed, will retaining it exact behavior, by
``unrolling'' one iteration of the ``Repeat'' loop, re-writing the ``if'''s in lines
\ref{alg1:startifs} through
\ref{alg1:endifs} to use the indicator functions $\indic_{i,t+1}$, and
...
\begin{algorithm} [h!]
  ...
  \begin{algorithmic}[1]
    ...
    \REPEAT
        ...
        \FOR{$i \in \swarm$}
          \label{alg2:fatitert}
          \STATE DISPATCH {$\fppos_{i.t} \leftarrow \ofunc(\ppos_{i,t})$} to a seperate processor
        \ENDFOR
        \FOR{$i \in \swarm$}
          \STATE $\nURand_{i,t} \Leftarrow RandomVector()$
          \STATE $\nURand_{i,t} \Leftarrow RandomVector()$
          \FOR{$c \in \caseset$}
            \STATE Compute $\specvel_{i,t+1}^c$ and $\specpos_{i,t+1}^c$.
	    Examples shown in equations~\eqref{eq:defvcasexn} and~\eqref{eq:defpcasexn}
            \label{alg2:fatitertp1}
            \STATE DISPATCH computation of $\specval_{i,t+1}^c$ to a seperate processor.
	    Example shown in equation~\eqref{eq:deffcasexn}.
	    \COMMENT Note that none of the computations, $\specvel_{i,t+1}^c$,
	    $\specpos_{i,t+1}^c$ and $\specval_{i,t+1}^c$ depend on $\fppos_{i.t}$.
          \ENDFOR
        \ENDFOR
        \STATE WAIT for all function evaluations dispatched above to complete
        \FOR{$i \in \swarm$}
          \FOR{$c \in \caseset$}
            \STATE Compute $\indic_{i,t+1}^c$
	           Examples shown in equation~\eqref{eq:deficasexn} and~\eqref{eq:defpcasexn}
	  \ENDFOR 
          \STATE Compute $\pvel_{i,t+1}$ as shown in equation~\eqref{eq:vel2update} 
          \STATE Compute $\ppos_{i,t+1}$ as shown in equation~\eqref{eq:pos2update}
          \STATE Compute $\fppos_{i,t+1}$ as shown in equation~\eqref{eq:val2update}
        \ENDFOR
