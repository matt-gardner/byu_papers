...
\sec{results} that in many, though not all, instances, a smaller swarm
...
There are also many ways to speculate several iterations ahead, and we again
only show results for one of them.  The combination of branches we tried uses
seven speculative evaluations per particle, matching the swarm size of the
original speculative algorithm.  The seven evaluations we used corresponded to
several iterations of branches \casepn\ and \casexn.  Two of the evaluations
were just one iteration ahead, four were two iterations ahead, and one was
three iterations ahead.  The evaluations that were one iteration ahead were
branches \casepn\ and \casexn; those that went two iterations ahead were formed
by taking either branch \casepn\ or \casexn and then branch \casepn\ or
\casexn; and the evaluation going three iterations ahead followed branch
\casepn\ on all three iterations.
...
a small swarm using a Complete topology.  We expect our methods to be perfect
for such functions, and our results show this intuition again to be correct.
First we compare the basic SEPSO algorithm to standard PSO.  In
\fig{basic-sphere} we can see that when both SEPSO and PSO are using a Random
topology, SEPSO clearly outperforms PSO.  However, the comparison between SEPSO
and PSO Complete is not as encouraging---PSO Complete performs slightly better.
...
Iters by far outperforms every other algorithm on this function.  Many Iters on
average finds values that are more than 40 orders of magnitude better than
standard PSO using the same number of processors.  This is shown in
...
PSO.  The only algorithm that performed better than PSO was Pick Best Pruned,
likely because of the larger swarm size available to it.

\begin{figure}
  \centering
  \includegraphics[width=.8\columnwidth]{schwefel2}
  \caption{Function Schwefel with a swarm that uses 240 processors per round of
  evaluations, comparing all speculative algorithms.}
  \label{fig:schwefel2}
\end{figure}
...
sometimes it doesn't.  Instead of showing average function value at each
...
are shown in \fig{griewank-manyiters}. (TODO: Rerun Many Iterations with the
right swarm size)
...
the swarm sometimes finds it and sometimes doesn't.  Since we have already
