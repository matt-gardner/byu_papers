...
subscripts on $\indic_{t+1}^{\casegen}$.
\begin{align}
...
the objective function took one hour. Suppose also that for each particle the
computations $\ofunc(\ppos_{t})$ and all of the $\ofunc(\specpos_{t+1}^{c})$
were sent to separate processors and completed in parallel.  Once the values
of $\ofunc(\ppos_{t})$ have been computed for all particles, the computation
needed to determine the values of the indicator functions $\indic_{t+1}^{c}$
and to complete iteration $t+1$ is inconsequential.  In this environment all
of the function evaluations needed for two iterations of PSO could be
completed in just one hour where as the conventional implementation would
require two hours for those same two iterations.
...
as in \eqref{eq:pos2update}.  After having the slaves evaluate the
objective function at all necessary positions, the master then decides which
position to accept for each particle, as in \eqref{eq:val2updatelong}.
