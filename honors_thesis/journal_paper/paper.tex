...
\begin{align}
\nonumber
	\specvel_{i,t+1}^{\casexn} & (\ppos_{i,t+1}, \leftn_{i,t+1}, \rightn_{i,t+1},
...
\begin{align}
\label{eq:defpcasexn}
	\specpos_{i,t+1}^{\casexn} & (\ppos_{i,t+1} ,\leftn_{i,t+1},\rightn_{i,t+1} ,\pbest_{i,t} ,\nbest_{i,t})= \\
\nonumber
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
          ...
          \STATE $\ofunc(\pbest_{i,t})\leftarrow \ofunc(\pbest_{i,t-1})$
          \IF {$\ofunc(\ppos_{i,t})<\ofunc(\pbest_{i,t-1})$}
            \STATE $\pbest_{i,t}\leftarrow \ppos_{i,t}$
            \STATE $\fpbest_{i,t}\leftarrow \fppos_{i,t}$
          \ENDIF
          \STATE $\nbest_{i,t}\leftarrow \nbest_{i,t-1}$
          \STATE $\ofunc(\nbest_{i,t})\leftarrow \ofunc(\nbest_{i,t-1})$
          \IF {$\ofunc(\fppos_{i,t})<\ofunc(\nbest_{i,t})$}
            \STATE $\pbest_{i,t}\leftarrow \ppos_{i,t}$
            \STATE $\ofunc(\pbest_{i,t})\leftarrow \ofunc(\ppos_{i,t})$
          \ENDIF
          \IF {$\ofunc(\leftn_{i,t})<\ofunc(nbest_{i,t})$}
            \STATE $\pbest_{i,t}\leftarrow \leftn_{i,t}$
            \STATE $\ofunc(pbest_{i,t})\leftarrow \ofunc(\leftn_{i,t})$
          \ENDIF
          \IF {$\ofunc(\rightn_{i,t})<\ofunc(\nbest_{i,t})$}
            \STATE $\pbest_{i,t}\leftarrow \rightn_{i,t}$
            \STATE $\ofunc(\pbest_{i,t})\leftarrow \ofunc(\rightn_{i,t}$
          \label{alg1:endifs}
          \ENDIF
...
%    \ENDFOR
    \REPEAT
        ...
        \FOR{$i \in \swarm$}
          \label{alg2:fatitert}
          \STATE DISPATCH the computation of {$\ofunc(\ppos_{i.t})$} to a seperate processor
        \ENDFOR
        \FOR{$i \in \swarm$}
          \STATE $\nURand_{i,t} \Leftarrow RandomVector()$
          \STATE $\nURand_{i,t} \Leftarrow RandomVector()$
	    ...
	    Examples shown in equations~\eqref{eq:defvcasexn} and~\eqref{eq:defpcasexn}
            ...
            \STATE DISPATCH computation of $\ofunc(\specpos_{i,t+1}^c)$ to a seperate processor.
	    Example shown in equation~\eqref{eq:deffcasexn}.
	    // Note that NONE of the computations, $\specvel_{i,t+1}^c$,
	    $\specpos_{i,t+1}^c$ and $\ofunc(\specpos_{i,t+1}^c)$ depend on $\ofunc(\ppos_{i.t})$.
          \ENDFOR
        \ENDFOR
