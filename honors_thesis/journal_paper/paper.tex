...
In this example it is important to note that we do not need the value of the objectve function at the current
point to to reduce the infinite number of possible next postions to just two alternatives.
Given the random sample, the next point detments only on the binary test and the
random sample is, by definition, idependent of the positions, and the vaues obtained.
The choice of the next position can be made post-hoc, after the value at the current position is obtained.
...
a set of infinitly many posible \emph{values} outcomes
...
By pruning the speculation to just paths that are statistically likely to reproduce the paths that are equivelent to PSO we can also 
...
$\caseset$ represents the set of all update cases.
...
The update equation is taken exactly from the standard update equation~\eqref{eq:velupdate}, but
...
\end{align}
We call these values, $\specpos^{c}) \ c \in\ \caseset$ (7 of them in this case where each particle has 2 neighbors) as ``speculative children''
...
Returning our attention to the computtaion of $\specpos$ in \eqref{eq:pos2update}and writing it with the parameters which were omitted above,
we obtain:
\begin{align}
\nonumber
...
\end{align}
In this form the important point to notice is that none of the $\specpos_{t+1}^{c} \ c \in\ \caseset$ depend upon $f(x_t)$
or any other objective function evaluation at $t$.
Thus PSO has been re-factored such that the algorithm can begin computing
all 7 of the objective function evaluations potentially needed in iteration $t+1$ before $f(x_t)$ is computed.
...
Once the evaluation of $f(\ppos_{t})$ is completed only one
...
of the $\ofunc(\specpos_{t+1}^{c}) \ c \in \caseset$ are the computationally difficult parts.

For example, suppose that unlimited processors were available and that the evaluation of 
