...
\title{\ \\ \LARGE\bf A Speculative Approach to Parallelization in Particle
Swarm Optimization%
...
\begin{keywords}
Parallel algorithms, optimization methods, particle swarm optimization.
\end{keywords}
...
same amount of time~\cite{mcnabb-2009-large-particle-swarms}.  In
...
To solve this problem, we apply the concept of speculative
decomposition~\cite{grama-2003-intro-to-parallel-computing} to particle swarm
...
relaxations make better use of the information obtained from the extra
exploration made by the speculative function evaluations.  We also explore the
idea that, like branch prediction in processors, we need not speculatively
evaluate \emph{all} possible future positions, we can accelerate the algorithm
even if we are just \emph{likely} to have guessed right.  By pruning the
speculation to just paths that are statistically likely to reproduce the paths
that are equivalent to PSO we can increase the swarm size without increasing
the number of speculative evaluations.  We also consider several recovery
strategies for cases where the pruned set of speculative evaluations does not
contain the evaluation that standard PSO would have done.  A further
improvement we explore is speculating several iterations ahead instead of just
one, which is made possible by pruning the number of speculative evaluations.
...
reproducing the behavior of the original algorithm.  \sec{setup} describes the
experiments we ran, and \sec{results} presents the results of those
experiments.  In \sec{conclusion} and \sec{future} we conclude and discuss
future work.
...
parallel evolution
strategies~\cite{rudolph-1991-distributed-evolution-strategies}.  In evolution
...
generation~\cite{beyer-2002-evolution-strategies}.  Our methods are similar,
...
all of our experiments, giving all algorithms the same number of function
evaluations.  Thus while we label our graphs with ``Rounds of Evaluations,''
this is equivalent to the number of function evaluations used by each algorithm
...
Using the same number of processors for each algorithm requires that our
speculative algorithms have a smaller swarm size than standard PSO.  For each
of the topologies we used, a particle has three neighbors including itself.  As
...
particle, we have $8p$ evaluations for every two iterations, where $p$ is the
number of particles in the speculative swarm.  The extra evaluations required
in our speculative approach would instead be used to evaluate particles in
regular PSO, so we compare swarms of size $p$ in speculative evaluation with
swarms of size $8p$ in regular PSO.

Where the Complete topology would normally be preferred, we use a Random
topology in our speculative algorithm, as Complete leads to an explosion in the
...
would be used even with standard PSO.  But, to be fair in our comparisons, we
compare to the standard PSO algorithm using both the Random topology and the
Complete topology (labeled PSO Random and PSO Complete in our results).  

We experimented with five common benchmark problems defined
in~\cite{herrera-2010-test-suite}: Rastrigin, Sphere, Schwefel 2.21, Griewank,
and Bohachevsky.  
...
The algorithms we compare are the standard PSO algorithm, our original
speculative approach (here labeled SEPSO simply for ease of reference), and the
four relaxations of SEPSO discussed in \sec{relax}.  In presenting our results,
we call the approach developed in \sec{pickbest} Pick Best.  The algorithms
described in \sec{pruning} through \sec{wrong} are called Pick Best Pruned and
Social Promotion Pruned, and the algorithm in \sec{manyiters} we call Many
Iterations.
...
$\frac{1}{3}p$ to compare to a swarm of size $p$ with standard PSO, instead of
$\frac{1}{8}p$ with other techniques.
...
up to at least 4000 particles~\cite{mcnabb-2009-large-particle-swarms}.
...
each had 30 particles, and the standard PSO algorithms had 240 particles.  As
expected, SEPSO and Pick Best converge quickly to worse local optima than
standard PSO does.  \fig{rastrigin} shows the results graphically.
...
intuition again to be correct.  First we compare the basic SEPSO algorithm to
standard PSO.  In \fig{basic-sphere} we can see that when both SEPSO and PSO
...
\fig{sphere-pickbest}, where SEPSO did not manage to perform as well as PSO
...
better than standard PSO using the same number of processors.  This is shown in
...
fail to perform as well as standard PSO because of their smaller swarm size.
In comparing SEPSO to standard PSO in \fig{schwefel1}, we can see that SEPSO
fails to perform any better than PSO with a Random topology, and it is by far
outperformed by PSO with a Complete topology.
...
perform worse than PSO Complete, with Many Iterations performing comparably to
PSO.  However, one of the speculative algorithms did perform better than PSO:
Pick Best Pruned.  It seems that the larger swarm size available because of
pruning was the cause of the improved performance.
...
optimum~\cite{bratton-2007-defining-a-standard-for-pso}.  Griewank has a global
optimum with a value of 0, and sometimes the swarm finds the optimum and
...
twice as fast as the original PSO without a significant loss of accuracy.
...
In order to show the premature convergence properties of the various algorithms
...
When we look at the performance of our Pick Best algorithm, a less intuitive
...
tried, on average four times faster than standard PSO.  However, it also gets
stuck and fails to find the optimum more than any other method.  The results
are shown in \fig{griewank-manyiters}.   \fig{griewank-manyiters} is also
interesting in that it highlights the trade-off between accuracy and speed in
the various algorithms at this swarm size.  The faster the algorithm finds the
...
Now that we have seen the effect that the various algorithms have on speed of
convergence and success rates, we show a result for using 800 processors, where
most of the algorithms have a 100\% success rate.  In this case, the best
...
which finds the optimum on average three times as fast as standard PSO.
...
\fig{bohachevsky} we can see that all of the speculative algorithms found the
optimum much quicker than PSO with a Random topology.  However, SEPSO was
slower than PSO Complete and got stuck around 15\% of the time.  Pick Best,
Pick Best Pruned, and Many Iterations all outperformed PSO Complete, though
Many Iterations only almost reached a 100\% success rate.
...
did not compare our methods against the state of the art in PSO research.  We
...
the No Free Lunch Theorem~\cite{wolpert-1997-nfl-for-optimization}.  We have
...
best across all functions, for all but one of the functions we looked at
\emph{some} kind of speculative evaluation outperforms standard PSO, often
dramatically.  Our purpose has been simply to show that in many cases
speculative techniques outperform additional particles as the swarm size gets
large; our results are conclusive in that regard.  We are arguing for the
speculative parallelization of PSO, not for one particular approach to it.
...
Swarm~\cite{mendes-2004-fully-informed-particle-swarm}.  While our methods will
not always be immediately applicable to every variant, we are confident that
some kind of speculative approach will be beneficial to the parallelization of
all forms of PSO, especially as the number of processors used gets into the
thousands.
