...
plot.show()
plot.write('sphere1.gpi')

ITERATIONS = 250
SWARMSIZE = 50

PICKBEST_FILE = '../sepso_pickbest/sphere/rand52particles'
PICKBEST13_FILE = '../sepso_pickbest/sphere/rand13particles2000iters'

pickbest_data = PSOData(open(PICKBEST_FILE))
pickbest13_data = PSOData(open(PICKBEST13_FILE))
pickbest_points = []
pickbest13_points = []
for i in xrange(1, ITERATIONS):
    evaluations = i * SWARMSIZE
    pickbest_points.append((evaluations, pickbest_data.average(i*2)))
    pickbest13_points.append((evaluations, pickbest13_data.average(i*8)))
...
plot.append(Points(pickbest_points, title='52 Particles', style='lines'))
plot.append(Points(pickbest13_points, title='13 Particles',
    style='lines'))

plot.show()
plot.write('sphere2.gpi')
