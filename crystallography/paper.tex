...
Crystallography is the science of determining the structure of a crystal.  This
is done by shining x-rays through the crystal and observing the patterns that
come out, generally consisting of intensities of light at various points in
space.  Those patterns are then compared to the output of a model of the
crystal -- the predicted intensities of light at the same points in space.  If
the two match, the model can be considered correct.  The model may consist of
the positions of all of the atoms in the crystal, the distortion coefficients
of the material, the length and width of nanopolar domains, or various other
parameters.  The task of finding the parameters to the model that best fit the
data is well suited to optimization techniques.  The function to be optimized
is an error function -- the average difference between the model output and the
data, for every data point.

There are commercial packages that readily solve some crystallography problems,
such as (ask Dr. Campbell), that uses simulated annealing for specific types of
problems.  Gradient descent is also a commonly used method for solving these
problems.  Particle swarm optimization (PSO), described in a later section, has
also been used, but only in limited contexts \cite{liu-anc05}.

To solve crystallography problems, one must fit the parameters of the model to
the data.  The more data is used, the more accurate the result will be, but
with large amounts of data the error function evaluation time can be
inhibitively large, easily reaching minutes or even hours.  In such cases,
performing any kind of optimization on a single machine is infeasible; parallel
techniques are needed.

We present a crystallography problem whose error function evaluation takes
several minutes. We then present our solution to the problem, using MRPSO, a
parallel particle swarm optimization implementation \cite{mcnabb-cec07}.  The
issues we encountered and solved dealing with this crystallography problem are
applicable to any problem whose function evaluation takes large amounts of
time.  

Section~\ref{sec:butterfly} introduces the crystallography problem we solved.
Section~\ref{sec:pso} explains the particle swarm optimization algorithm, and
section~\ref{sec:mapreduce} explains MapReduce and PSO in the context of
MapReduce.  Section~\ref{sec:results} then describes our solution to the
problem.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Butterfly Problem
\section{Crystallography Problem}
\label{sec:butterfly}
TODO.
...
\section{Issues with Optimization}
\label{sec:issues}
TODO.

In the course of solving this problem, it was discovered that a few of the
variables were correlated.  In initial attempts at optimization, the algorithm
never converged -- the function value reached a certain point and stayed close
to that point while a couple of the parameters fluctuated greatly in a
correlated fashion.  Figure (make figure) shows a range of values that two of
parameters took on, without changing the function value more than (check
number) .5, or this \%.  Upon noticing this empirical correlation, the function
was more closely examined, and a theoretical correlation was also discovered.
Given that the parameters were correlated and the function had too many free
variables, one of the parameters was fixed at a reasonable value, and the
optimization algorithm successfully and quickly solved the problem.

The topology that was used in the PSO algorithm was the gbest topology.
Experiments were done with lbest to determine which was more efficient, and it
was discovered that for this problem gbest was better.  Averages over three
runs are shown in Figure (make figure) for the two topologies.  

Work has been done showing that when running PSO in parallel with large swarms
more complicated topologies can do just as well as gbest while decreasing the
amount of communication required~\cite{mcnabb-gecco09}.  However, as the
function evaluation for this problem takes several minutes, the amount of
communication time for gbest is negligible when compared to the function
evaluation time.  The communication overhead for each iteration was discovered
to be about 8 seconds for the MRPSO implementation used.
...
\bibliography{%
    ...
    \bib{pso/crystallography/liu-anc05}}
